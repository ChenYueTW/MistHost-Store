...
Ubuntu / Pterodactyl 上建議把 SMTP 放在 Variables，避免 `config.json` 被重新上傳覆蓋：

- `SMTP_ENABLED=true`
- `SMTP_HOST=smtp.gmail.com`
- `SMTP_PORT=587`
- `SMTP_SECURE=false`
- `SMTP_FROM=MistHost <service@misthost.net>`
- `SMTP_USER=service.misthost@gmail.com`
- `SMTP_PASS=Google App Password`

如果 `config.json` 不在預設根目錄，可以用 `CONFIG_PATH=/home/container/config.json` 指定路徑。
如果要指定 SQLite 檔案位置，可以用 `DATABASE_FILE=data/store.db` 或其他持久化路徑。
